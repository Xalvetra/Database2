...
CAST(CURRENT_TIMESTAMP AS date)AS
currentdate, CAST(CURRENT_TIMESTAMP AS time)AS
currenttime, YEAR(CURRENT_TIMESTAMP) AS
currentyear, MONTH(CURRENT_TIMESTAMP) AS
currentmonth, DAY(CURRENT_TIMESTAMP) AS
currentday, DATEPART(WEEK, CURRENT_TIMESTAMP) AS
currentweeknumber, DATENAME(MONTH, CURRENT_TIMESTAMP) AS
currentmonthname;
---------------------------------------------------------------------
-- Task 2
--  
...
currentmonth, DAY(CURRENT_TIMESTAMP) AS
currentday, DATEPART(WEEK, CURRENT_TIMESTAMP) AS
currentweeknumber, DATENAME(MONTH, CURRENT_TIMESTAMP) AS currentmonthname;
...
SELECT
CONVERT(DATE, '1945-08-17') AS somedate
SELECT
PARSE ('1945-08-17' as date) AS somedate
SELECT
FORMAT (19990911, '####-##-##') AS somedate
-----------------------------------------------------------------------
SELECT DATEADD(MONTH, +3, GETDATE()) AS threemonths
SELECT DATEDIFF(DAY, GETDATE(), DATEADD(month, +3, GETDATE())) AS Diffday
SELECT DATEDIFF(WEEK, '1945-08-17', '2018-10-01') AS diffweeks
SELECT DATEADD(Day,-DATEPART(Day, GETDATE()) +1, GETDATE()) AS firstday
